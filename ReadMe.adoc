...
Small ESP32-based gas leakage detection project that reads an MQ-series gas sensor, reports PPM on a 16x2 I2C LCD and (optionally) to Arduino Cloud. The sketch includes a simple alarm sound via wired speaker and Pushbullet powered mobile and desktop device notification when gas levels rise.

image::assests/photo_6159094900922716418_y.jpg[Gas leakage detection setup, width=600]
image::assests/6179310908258062961.jpg[Gas leakage detection summary and graph, width=600]
image::assests/6179310908258062959.jpg[Gas leakage detection alarm, width=600]
...
- Plays alarm audio when gas levels exceed normal air limits
- Regulated Device notification with 60/20 sec break accrodingly
- Filtered alarm to get rid of uncertain alarm
...
- Speaker connected with amplifier module
...
- Amplifier (+) -> GPIO25, Amplifier (+) -> Speaker (+)
...
Please Download `Arduino IOT Remote` and `PushBullet` mobile app and set your configurations and everything properly to get best outcome and you can access from anywhere.
