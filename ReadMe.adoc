...
Small ESP32-based gas leakage detection project that reads an MQ-series gas sensor, reports PPM on a 16x2 I2C LCD and (optionally) to Arduino Cloud. The sketch includes a simple alarm sound via the ESP32 DAC when gas levels rise.
...
- Plays alarm audio via ESP32 DAC when gas levels exceed thresholds
...
- Small speaker or amplifier connected to the ESP32 DAC pin (optional)
...
- Speaker / DAC output -> GPIO25 (DAC output used to play PCM samples)
...
5. Select the correct ESP32 board and COM port, then upload.
...
- "Error: RO is 0 !!" — indicates RO calibration failed; re-check wiring and ensure the sensor warmed up before calibration.
- "Error: Sensor Disconnected?" — check analog wiring and power to the sensor.
- If readings are unexpectedly low/high, verify sensor VCC (3.3V) and the ADC reference on your board.
